# =============================================================
#  vlf_visualizza_nsy_v3.py
#  GAESID — VLF SID Monitor · Stazione di Torino, Mirafiori Sud
# =============================================================
#
#  Versione  : 3.0  (basata su Visualizza_20_NSY_con_XRAY_e_Flares.py)
#  Autore    : Gaetano  <https://gaetano-projects.github.io>
#  Licenza   : MIT — uso libero con attribuzione
#
#  Descrizione
#  -----------
#  Script di visualizzazione per il monitoraggio VLF del trasmettitore
#  navale NSY (Niscemi, Sicilia) a 45,9 kHz, percorso Torino–Niscemi
#  (~1200 km, prevalentemente marittimo). Dati acquisiti con SDRplay
#  RSP1B e antenna loop ad aria costruita artigianalmente.
#
#  Funzionalità principali
#  -----------------------
#  · Lettura CSV VLF in formato GAESID (GAESID_NSY_YYYY-MM-DD.csv)
#  · Smoothing Savitzky-Golay configurabile (1–∞ secondi)
#  · Overlay flusso X-ray GOES-R (canale B, 0.1–0.8 nm) con asse log
#    e fasce colorate per classi A/B/C/M/X
#  · Download automatico flare NOAA/DONKI con cache locale JSON
#  · Download automatico X-ray con cache locale CSV
#  · Marcatori flare sul grafico (colore per classe, simbolo BTL)
#  · Selezione data e smoothing tramite finestre grafiche tkinter
#  · Inserimento manuale flare beyond-the-limb (tasto M)
#  · Navigazione tra giorni (tasti A/D) e selezione data (tasto I)
#  · Esportazione PNG con nome file automatico (tasto P)
#  · Alba/tramonto calcolati con pyephem per la posizione della stazione
#
#  Comandi da tastiera (finestra grafico)
#  ---------------------------------------
#  A  →  Giorno precedente
#  D  →  Giorno successivo
#  I  →  Selezione data tramite calendario
#  M  →  Inserisci flare manuale (beyond-the-limb)
#  P  →  Salva grafico corrente come PNG
#  R  →  Aggiorna grafico, flare e X-ray (forza re-download)
...
#  Dipendenze
#  ----------
#  pip install requests matplotlib numpy scipy ephem tkcalendar
#
# =============================================================
...
# =============================================================
#  CONFIGURAZIONE — adattare alle proprie directory
# =============================================================

# Directory dove cercare i file CSV giornalieri.
# Lo script cerca prima in CARTELLA_GIORNALIERA, poi in CARTELLA_STORICO.
# I file devono avere il nome: GAESID_NSY_YYYY-MM-DD.csv
CARTELLA_GIORNALIERA = r"D:\Gaetano\SID\dati\Sdruno"
CARTELLA_STORICO     = r"D:\Gaetano\SID\dati\Sdruno\Storico_NSY"

# Cartella per la cache locale del flusso X-ray GOES (file CSV per giorno)
CARTELLA_XRAY   = r"D:\Gaetano\SID\dati\Sdruno\Storico_NSY\XRAY"

# Cartella per la cache locale dei flare NOAA/DONKI (file JSON per giorno)
CARTELLA_FLARES = r"D:\Gaetano\SID\dati\Sdruno\Storico_NSY\Flares"

# File JSON per i flare inseriti manualmente (beyond-the-limb, ecc.)
FILE_FLARE_MANUALI = os.path.join(CARTELLA_STORICO, "flare_manuali.json")

# Coordinate e quota della stazione (per calcolo alba/tramonto)
# Defaults: Torino, Mirafiori Sud
STAZIONE_LAT       = '45.07'   # gradi Nord
STAZIONE_LON       = '7.68'    # gradi Est
STAZIONE_ELEVATION = 240        # metri s.l.m.
...
def calcola_alba_tramonto(data_utc):
    ...
    osservatore.lat       = STAZIONE_LAT
    osservatore.lon       = STAZIONE_LON
    osservatore.elevation = STAZIONE_ELEVATION
...
def path_json_flare(data_utc):
    """Restituisce il path del file JSON locale per i flare della data indicata."""
...
def leggi_flare_cache(data_utc):
    """
    Legge i flare dalla cache locale JSON.
    Restituisce lista di flare o None se il file non esiste.
    """
    oggi_utc = datetime.now(timezone.utc).date()
    if data_utc.date() == oggi_utc:
        return None   # oggi si scarica sempre online
...
def salva_flare_cache(data_utc, flares):
    """
    Salva i flare in cache locale JSON per riuso futuro.
    Non salva mai i flare del giorno corrente.
    Non salva i flare manuali (sono già in flare_manuali.json).
    """
    oggi_utc = datetime.now(timezone.utc).date()
    if data_utc.date() == oggi_utc:
        return   # oggi non si mette in cache
    ...
    try:
        ...
        for f in flares:
            if f.get('manuale'):
                continue   # i flare manuali non vanno in cache
            dati.append({
        ...
        if dati:
            print(f"Flare salvati in cache: {len(dati)} eventi ({path})")
        else:
            print(f"Nessun flare: file cache vuoto salvato ({path})")
    except Exception as e:
...
def disegna_grafico():
    ...
    # --- Flare: usa cache locale se disponibile, altrimenti scarica ---
    flares_cache = leggi_flare_cache(data_utc)
    if flares_cache is not None:
        # Aggiunge i flare manuali alla lista dalla cache
        flares_manuali = [
            f for f in carica_flare_manuali()
            if f['picco'].date() == data_utc.date()
        ]
        ...
        # Salva in cache per uso futuro (escluso oggi)
        # Salva anche se lista vuota: evita download ripetuti per giorni senza flare
    ...
    ax.set_title(
        ...
        f"Tasti:  A = prec.  |  D = succ.  |  I = data  |  M = flare  |  "
        f"S = smooth lista  |  V = smooth libero  |  "
...
def aggiorna_titolo_mancante(data_utc):
    ...
    ax.set_title(
        ...
        f"I = inserisci data  |  S = smooth lista  |  V = smooth libero  |  "
        f"R = Aggiorna  |  P = salva PNG  |  Q = esci",
        color='white', fontsize=10, fontweight='bold', pad=10
    )
...
# =============================================================
    ...
    if tasto == 'r':
        ...
        # Scegli smoothing dalla lista di valori standard
        nuovo = apri_dialogo_smooth_lista()
        if nuovo is not None and nuovo != stato['smooth']:
            stato['smooth'] = nuovo
            print(f"Smoothing impostato a: {nuovo} s")
            disegna_grafico()
        else:
            print("Smoothing invariato.")

    elif tasto == 'v':
        # Inserisci smoothing manuale (valore libero)
...
def scegli_smoothing_iniziale():
    ...
    tk.Label(root, text="Scegli la finestra di smoothing (secondi)", 
             bg='#0d0d1a', fg='#00FF99', font=('Consolas', 11, 'bold')).pack(pady=(14, 6), padx=20)
    ...
    etichette = {1: "Nessuno\n(1 s)", 15: "15 s", 30: "30 s",
                 60: "60 s", 90: "90 s", 120: "120 s"}

    btn_sel = {'btn': None}

    def seleziona(val, btn):
        if btn_sel['btn']:
            btn_sel['btn'].config(bg='#1e1e3a', fg='white', relief='flat')
        btn.config(bg='#00FF99', fg='black', relief='sunken')
        btn_sel['btn'] = btn
        risultato['valore'] = val

    for val in VALORI_SMOOTH_STANDARD:
        b = tk.Button(frame_std, text=etichette.get(val, str(val)),
                      width=9, height=2, bg='#1e1e3a', fg='white',
    ...
    tk.Button(root, text="Conferma", command=root.destroy,
              bg='#00FF99', fg='black', font=('Consolas', 11, 'bold'),
...
def apri_dialogo_smooth_lista():
    ...
    tk.Label(root, text=f"Scegli smoothing\n(Attuale: {corrente} s)",
             bg='#0d0d1a', fg='#00FF99', font=('Consolas', 11, 'bold')).pack(pady=12)
    ...
    def seleziona(val, btn):
        if btn_attivo['btn'] is not None:
            btn_attivo['btn'].config(bg='#1e1e3a', fg='white', relief='flat')
        btn.config(bg='#00FF99', fg='black', relief='sunken')
        btn_attivo['btn'] = btn
        risultato['valore'] = val
    ...
    for val in VALORI_SMOOTH_STANDARD:
        ...
        b = tk.Button(frame, text=etichette.get(val, str(val)),
                      width=9, height=2, bg=bg_i, fg=fg_i,
    ...
    tk.Button(frame_btn, text="Annulla", command=annulla,
              bg='#333355', fg='white', font=('Consolas', 10),
              relief='flat', padx=12, pady=4).pack(side='left', padx=8)
...
def apri_dialogo_smooth_manuale():
    """
    Tasto V — Finestra con campo testo per inserire un valore libero
    di smoothing in secondi. Restituisce il valore scelto o None se annullato.
    """
    root = tk.Tk()
    ...
    corrente = stato['smooth']
    tk.Label(root, text="Inserisci il valore di smoothing (secondi)",
             bg='#0d0d1a', fg='#00FF99',
             font=('Consolas', 11, 'bold')).pack(pady=(14, 4), padx=20)
    tk.Label(root, text=f"Valore attuale: {corrente} s   (1 = nessuno smoothing)",
             bg='#0d0d1a', fg='#aaaacc',
    ...
    entry = tk.Entry(frame_e, width=8,
                     bg='#1e1e3a', fg='white', insertbackground='white',
                     font=('Consolas', 14), justify='center', relief='flat')
    entry.insert(0, str(corrente))
    entry.selection_range(0, tk.END)
    entry.pack(side='left')
    entry.focus_set()
    tk.Label(frame_e, text=" s", bg='#0d0d1a', fg='#aaaacc',
             font=('Consolas', 12)).pack(side='left')

    lbl_err = tk.Label(root, text="", bg='#0d0d1a', fg='#FF6347',
                       font=('Consolas', 9))
    ...
    def conferma():
        try:
            ...
            if val < 1:
                lbl_err.config(text="Il valore deve essere >= 1")
                return
            risultato['valore'] = val
            root.destroy()
        except ValueError:
    ...
    tk.Button(frame_btn, text="Annulla", command=root.destroy,
              bg='#333355', fg='white', font=('Consolas', 10),
...
# Selezione smoothing iniziale
finestra_smooth = scegli_smoothing_iniziale()
print(f"Smoothing iniziale: {finestra_smooth} s")

fig, ax = plt.subplots(figsize=(16, 6))
fig.patch.set_facecolor('#0d0d1a')
ax.set_facecolor('#0d0d1a')

# Disabilita tasti matplotlib di default
for tasto in ['p','q','s','v','c','a','d','g','h','r','k','l','o','f','x']:
    for keymap in list(plt.rcParams.keys()):
        if keymap.startswith('keymap.') and tasto in plt.rcParams[keymap]:
            try:
                plt.rcParams[keymap].remove(tasto)
            except:
                pass
