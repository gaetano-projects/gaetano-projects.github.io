...
# ===== GIORNI PER L'ADDESTRAMENTO (modifica con le tue date) =====
GIORNI_TRAINING = [
        '2026-06-16',
        '2026-06-15',
        '2026-06-14',
        ...
        '2026-06-07',
        '2026-05-19',
    # Aggiungi qui tutte le date che vuoi
]
...
# Cartella dove salvare i file JSON delle risposte di addestramento
CARTELLA_RISPOSTE = r"D:\Gaetano\SID\dati\Sdruno\Storico_NSY\Training"
os.makedirs(CARTELLA_RISPOSTE, exist_ok=True)
...
# Variabile globale per il modello AI (da caricare/addestrare)
modello_ai = None
...
def applica_smoothing(valori, finestra):
    if not valori or len(valori) == 0:
        return valori

    dati = np.array(valori, dtype=float)
    n = len(dati)
    ...
    # --- FASE 1: Hampel vettorizzato (Rimuove gli spike di rumore senza rovinare i picchi veri) ---
    finestra_spike = min(finestra_spike, n)
    if finestra_spike >= 3 and n > 10:
        meta = finestra_spike // 2
        mediana = median_filter(dati, size=finestra_spike, mode='nearest')
        mean_filtered = uniform_filter(dati, size=finestra_spike, mode='nearest')
        mean_sq_filtered = uniform_filter(dati**2, size=finestra_spike, mode='nearest')
        varianza = mean_sq_filtered - mean_filtered**2
        std_dev = np.sqrt(np.maximum(varianza, 0))
        mask_outlier = np.abs(dati - mediana) > soglia_hampel * std_dev
        mask_outlier[0:meta] = False
        mask_outlier[-meta:] = False
        
        num_rimossi = np.sum(mask_outlier)
        if num_rimossi > 0:
            print(f"🔧 Hampel: rimossi {num_rimossi} outlier su {n} campioni")
        
        dati[mask_outlier] = mediana[mask_outlier]

    # --- FASE 2: RIMOSSA IL FILTRO MEDIANO ---
    # (Prima c'era un "medfilt" qui che rovinava la salita rapidissima dei SID.
    #  L'Hampel sopra è già sufficiente per pulire il rumore, il Savitzky-Golay sotto
    #  farà il resto senza distruggere i fronti di salita).

    # --- FASE 3: Savitzky-Golay (Il vero smoothing che mantiene la forma del picco) ---
    if finestra > 1:
        finestra_sg = finestra if finestra % 2 == 1 else finestra + 1
        if finestra_sg < 5:
            finestra_sg = 5
        polyordine = min(2, finestra_sg - 1)
        try:
            dati = savgol_filter(dati, window_length=finestra_sg, polyorder=polyordine)
        except Exception:
            kernel = np.ones(finestra_sg) / finestra_sg
            dati = np.convolve(dati, kernel, mode='same')
        meta = finestra_sg // 2
        if len(dati) > meta * 2:
            dati[:meta] = dati[meta]
            dati[-meta:] = dati[-meta - 1]

    return dati.tolist()

# =============================================================
# RILEVAMENTO AUTOMATICO SID (picchi nel segnale VLF)
# Versione 2 — correzioni applicate rispetto alla v1:
#
#  FIX 1: frequenza di campionamento calcolata dai timestamps
#          e usata per normalizzare order (argrelmax) e le
#          soglie di durata (durata_min / durata_max).
#
#  FIX 2: baseline calcolata come percentile 10 (non minimo)
#          per robustezza contro spike negativi isolati.
#
#  FIX 3: area calcolata rispetto alla baseline (non al picco),
#          così è sempre positiva e proporzionale all'energia
#          del SID.
#
#  FIX 4: soglia per trovare inizio/fine portata al 50%
#          dell'ampiezza (half-width), stima più standard e
#          meno sensibile al recupero lento del segnale.
#
#  FIX 5: warning in log quando inizio o fine del SID non
#          vengono trovati entro la finestra di ricerca (±30 min).
# =============================================================

def rileva_sid(timestamps, valori, data_utc,
               finestra_picchi_s=300,   # secondi: distanza minima tra due picchi
               finestra_baseline_s=600, # secondi: finestra pre-picco per la baseline
               ampiezza_min=2,        # dB: ampiezza minima del picco rispetto baseline
               durata_min_s=60,         # secondi: durata minima del SID
               durata_max_s=3600):      # secondi: durata massima del SID
    """
    Rileva SID cercando massimi locali nel segnale VLF smoothed a 60 s.

    La crescita diurna viene esclusa da:
      - finestra diurna (alba+30min … tramonto-30min)
      - ampiezza < ampiezza_min (default 2 dB)
      - durata > durata_max_s (default 1 ora)

    Parametri tutti in SECONDI; la conversione a indici avviene
    internamente dopo aver misurato il campionamento reale del CSV.

    Restituisce una lista di dizionari con:
      picco     – datetime UTC del massimo
      ampiezza  – dB rispetto alla baseline
      durata    – secondi (half-width al 50 % dell'ampiezza)
      area      – dB·s (integrale del bump rispetto alla baseline)
      inizio    – datetime UTC del fronte di salita (50 %)
      fine      – datetime UTC del fronte di discesa (50 %)
      baseline  – valore dB stimato pre-picco
    """
    ...
    # ------------------------------------------------------------------
    # FIX 1 — frequenza di campionamento reale
    # ------------------------------------------------------------------
    # Calcola dt mediano tra campioni consecutivi (secondi).
    # Gestisce sia timestamp datetime che float (epoch).
    ts = np.array(timestamps)
    if hasattr(ts[0], 'total_seconds'):
        # differenze già timedelta
        diffs = np.array([(ts[i+1] - ts[i]).total_seconds() for i in range(min(200, len(ts)-1))])
    elif hasattr(ts[0], 'timestamp'):
        # datetime objects
        diffs = np.array([(ts[i+1] - ts[i]).total_seconds() for i in range(min(200, len(ts)-1))])
    else:
        # float epoch
        diffs = np.diff(ts[:201].astype(float))

    dt = float(np.median(diffs[diffs > 0]))   # secondi per campione
    if dt <= 0:
        dt = 1.0  # fallback

    # Converti finestre da secondi a campioni
    order_campioni       = max(3, int(round(finestra_picchi_s  / dt)))
    base_campioni        = max(10, int(round(finestra_baseline_s / dt)))
    ricerca_campioni     = max(10, int(round(1800 / dt)))   # ±30 min per trovare inizio/fine
    durata_min_camp      = durata_min_s  / dt
    durata_max_camp      = durata_max_s  / dt

    print(f"   📐 Campionamento: {dt:.1f} s/campione  "
          f"| order={order_campioni} camp  "
          f"| baseline={base_campioni} camp  "
          f"| durata [{durata_min_s}s … {durata_max_s}s]")

    # ------------------------------------------------------------------
    # Alba / tramonto al punto medio path NSY–GAESID
    # ------------------------------------------------------------------
    ...
    print(f"   🌞 Finestra diurna: {alba_stabile.strftime('%H:%M')} – "
          f"{tramonto_stabile.strftime('%H:%M')} UTC")

    # ------------------------------------------------------------------
    # Smoothing fisso 60 s (indipendente dalla scelta interattiva)
    # ------------------------------------------------------------------
    valori_smooth = applica_smoothing(valori, 60)
    dati          = np.array(valori_smooth, dtype=float)
    n             = len(dati)
    ...
    # ------------------------------------------------------------------
    picchi = argrelmax(dati, order=order_campioni)[0]
    print(f"\n🔍 PICCHI TROVATI (order={order_campioni} camp ≈ {finestra_picchi_s} s): {len(picchi)}")
    ...
    for p in picchi:
        ...
        # ------------------------------------------------------------------
        # FIX 2 — baseline come percentile 10 della finestra pre-picco
        # ------------------------------------------------------------------
        inizio_base = max(0, p - base_campioni)
        finestra_pre = dati[inizio_base:p]

        if len(finestra_pre) < 5:
            continue  # troppo vicino all'inizio del file

        baseline = float(np.percentile(finestra_pre, 10))

        # Se l'ampiezza risultante è ancora piccola, allarga la finestra a 20 min
        if picco_val - baseline < 1.0:
            inizio_base2 = max(0, p - int(round(1200 / dt)))
            finestra_pre2 = dati[inizio_base2:p]
            if len(finestra_pre2) >= 5:
                baseline = float(np.percentile(finestra_pre2, 10))

        ampiezza = picco_val - baseline

        # --- Filtra ampiezza minima ---
        if ampiezza < ampiezza_min:
            continue

        # ------------------------------------------------------------------
        # FIX 4 — soglia al 50 % dell'ampiezza (half-width)
        # ------------------------------------------------------------------
        livello_soglia = baseline + ampiezza * 0.5

        # Ricerca inizio (fronte di salita)
        inizio_idx  = p          # default: rimane al picco se non trovato
        trovato_ini = False
        for i in range(p, max(0, p - ricerca_campioni), -1):
            if dati[i] <= livello_soglia:
                inizio_idx  = i
                trovato_ini = True
                break

        # Ricerca fine (fronte di discesa)
        fine_idx    = p          # default: rimane al picco se non trovato
        trovato_fin = False
        for i in range(p, min(n, p + ricerca_campioni)):
            if dati[i] <= livello_soglia:
                fine_idx    = i
                trovato_fin = True
                break
        ...
        # ------------------------------------------------------------------
        # FIX 5 — warning se inizio o fine non trovati
        # ------------------------------------------------------------------
        if not trovato_ini:
            print(f"   ⚠️  Picco {picco_time.strftime('%H:%M:%S')}: fronte di salita non trovato "
                  f"entro {ricerca_campioni} camp (≈30 min) — SID scartato")
            continue
        if not trovato_fin:
            print(f"   ⚠️  Picco {picco_time.strftime('%H:%M:%S')}: fronte di discesa non trovato "
                  f"entro {ricerca_campioni} camp (≈30 min) — SID scartato")
            continue

        # ------------------------------------------------------------------
        # FIX 1 (cont.) — durata in secondi, non in campioni
        # ------------------------------------------------------------------
        durata_camp = fine_idx - inizio_idx
        durata_sec  = durata_camp * dt

        if durata_sec < durata_min_s or durata_sec > durata_max_s:
            continue

        # ------------------------------------------------------------------
        # FIX 3 — area rispetto alla baseline (sempre positiva)
        # ------------------------------------------------------------------
        area = float(np.trapezoid(dati[inizio_idx:fine_idx + 1] - baseline, dx=dt))

        sid_list.append({
            'picco':     ts_arr[p],
            'ampiezza':  float(ampiezza),
            'durata':    float(durata_sec),
            'area':      float(area),
            'inizio':    ts_arr[inizio_idx],
            'fine':      ts_arr[fine_idx],
            'baseline':  float(baseline),
        })

    print(f"   ✅ SID finali dopo filtri: {len(sid_list)}")
    return sid_list

def carica_giorni_training(lista_date, cartella=CARTELLA_STORICO, prefisso="GAESID_NSY_",
                           pulisci=True, finestra_pulizia_s=1800):
    """
    Carica i CSV dei giorni selezionati per l'addestramento.
    Usa la stessa logica di lettura di leggi_csv_sid per evitare errori di parsing.
    """
    df_list = []
    for data_str in lista_date:
        file_path = os.path.join(cartella, f"{prefisso}{data_str}.csv")
        try:
            # --- LEGGI IL FILE COME FA leggi_csv_sid ---
            timestamps = []
            valori = []
            ...
            if len(timestamps) < 10:
                print(f"⚠️ File {data_str} ha meno di 10 campioni validi, saltato.")
                continue

            # --- CREA DATAFRAME ---
            df_giorno = pd.DataFrame({
                'timestamp': timestamps,
                'valore': valori
            })

            # --- PULIZIA (rimuove i SID) ---
            if pulisci and len(df_giorno) > 10:
                dt = (df_giorno['timestamp'].iloc[1] - df_giorno['timestamp'].iloc[0]).total_seconds()
                finestra_camp = max(3, int(finestra_pulizia_s / dt))
                if finestra_camp % 2 == 0:
                    finestra_camp += 1
                valori_clean = medfilt(df_giorno['valore'].values, kernel_size=finestra_camp)
                df_giorno['valore'] = valori_clean
                print(f"   🧹 Pulito {data_str} (finestra {finestra_pulizia_s}s)")

            df_list.append(df_giorno[['timestamp', 'valore']])
            print(f"✅ Caricato training: {data_str} ({len(df_giorno)} campioni)")

        except FileNotFoundError:
            print(f"⚠️ File non trovato per {data_str}, saltato.")
        except Exception as e:
            print(f"❌ Errore leggendo {data_str}: {e}")

    if not df_list:
        print("⚠️ Nessun giorno di training caricato. Uso fallback (stesso giorno).")
        return None
    return pd.concat(df_list, ignore_index=True)

def unifica_picchi_vicini(sid_list):
    if len(sid_list) <= 1: return sid_list
    sid_list.sort(key=lambda s: s['inizio'])
    merged = []
    current = sid_list[0]
    for next_sid in sid_list[1:]:
        # Unisci solo se gli intervalli si sovrappongono
        if next_sid['inizio'] <= current['fine']:
            # unisci
            if next_sid['ampiezza'] > current['ampiezza']:
                current['picco'] = next_sid['picco']
                current['ampiezza'] = next_sid['ampiezza']
                current['baseline'] = next_sid['baseline']
            current['inizio'] = min(current['inizio'], next_sid['inizio'])
            current['fine'] = max(current['fine'], next_sid['fine'])
            current['durata'] = (current['fine'] - current['inizio']).total_seconds()
            current['area'] += next_sid['area']
        else:
            merged.append(current)
            current = next_sid
    merged.append(current)
    merged = [s for s in merged if 60 <= s['durata'] <= 3600]
    return merged

def rileva_sid_con_ai(timestamps, valori, data_utc,
                      df_addestramento=None,
                      soglia_ampiezza=0.8,
                      distanza_min_s=300):
    """
    Rileva SID usando Random Forest per apprendere la curva di quiete.
    Include filtro notte/giorno (esclude i picchi notturni).
    """
    import ephem
    from datetime import timedelta

    # 1. Crea DataFrame del giorno target
    df = pd.DataFrame({
        'timestamp': timestamps,
        'valore': valori
    })

    # 2. Funzione interna per creare le feature temporali (riusabile)
    def crea_features(df_in):
        ...
        # Feature circolari per evitare il salto 23:59 -> 00:00
        ...
        # Trend lineare della giornata (per la salita del mattino)
    ...
    # 3. Addestramento del modello o fallback locale
    if df_addestramento is not None:
        df_train = crea_features(df_addestramento)
        X_train = df_train[features].values
        y_train = df_train['valore'].values
        print(f"   🤖 Addestramento AI su {len(df_train)} campioni di training (giorni selezionati)")
        
        model = RandomForestRegressor(
            n_estimators=100,      # più alberi per stabilità
            max_depth=4,           # solo trend lento
            min_samples_leaf=60,   # almeno 60 campioni per foglia (1 minuto a 1 Hz)
            random_state=42,
            n_jobs=-1
        )
        model.fit(X_train, y_train)
        df['quiete'] = model.predict(X_pred)
    else:
        # Nessun dato storico: stima locale con percentile mobile
        print("   ℹ️ Nessun training storico — uso percentile 10 mobile (60 min)")
        dt_sec = (timestamps[1] - timestamps[0]).total_seconds()
        df['quiete'] = sid_ai.stima_quiete_locale(df, finestra_minuti=60, dt_secondi=dt_sec)

    # 4. Calcola il residuo (questo prima causava il crash, ora è corretto)
    df['residuo'] = df['valore'] - df['quiete']

    # 5. Rileva i picchi nel RESIDUO
    dt = (timestamps[1] - timestamps[0]).total_seconds()
    distanza_min_camp = int(distanza_min_s / dt)

    picchi_idx, _ = find_peaks(
        df['residuo'].values,
        height=soglia_ampiezza,
        distance=distanza_min_camp,
        prominence=soglia_ampiezza * 0.4
    )

    print(f"   📈 Picchi trovati nel residuo (soglia {soglia_ampiezza}): {len(picchi_idx)}")

    # ==== CALCOLA ALBA E TRAMONTO ====
    osservatore = ephem.Observer()
    osservatore.lat = '41.1'   # punto medio NSY-GAESID
    osservatore.lon = '11.1'
    osservatore.elevation = 100
    osservatore.date = data_utc.strftime('%Y/%m/%d 00:00:00')
    osservatore.pressure = 0
    ...
    # Finestra diurna: alba + 30 minuti, tramonto - 30 minuti
    alba_stabile = alba + timedelta(minutes=30)
    tramonto_stabile = tramonto - timedelta(minutes=30)
    print(f"   🌞 Finestra diurna: {alba_stabile.strftime('%H:%M')} – {tramonto_stabile.strftime('%H:%M')} UTC")

    sid_list = []

    for idx in picchi_idx:
        picco_time = df['timestamp'].iloc[idx]
        ampiezza = df['residuo'].iloc[idx]

        # ==== FILTRO NOTTE ====
        if picco_time < alba_stabile or picco_time > tramonto_stabile:
            continue

        soglia_50 = ampiezza * 0.5

        # Trova inizio (cerca all'indietro fino a 30 minuti)
        inizio_idx = idx
        for i in range(idx, max(0, idx - int(1800/dt)), -1):
            if df['residuo'].iloc[i] <= soglia_50:
                inizio_idx = i
                break

        # Trova fine (cerca in avanti fino a 30 minuti)
        fine_idx = idx
        for i in range(idx, min(len(df), idx + int(1800/dt))):
            if df['residuo'].iloc[i] <= soglia_50:
                fine_idx = i
                break

        # Calcola durata e area
        durata_sec = (df['timestamp'].iloc[fine_idx] - df['timestamp'].iloc[inizio_idx]).total_seconds()
        area = np.trapezoid(df['residuo'].iloc[inizio_idx:fine_idx+1], dx=dt)
        ...
        sid_list.append({
            ...
            'durata': float(durata_sec),
            'area': float(area),
            'inizio': df['timestamp'].iloc[inizio_idx],
            'fine': df['timestamp'].iloc[fine_idx],
            'baseline': float(df['quiete'].iloc[idx]),
        })

    # ==== UNISCE PICCHI VICINI ====
    sid_list = unifica_picchi_vicini(sid_list)

    # Filtra anche per durata minima/massima
    sid_list = [s for s in sid_list if 60 <= s['durata'] <= 3600]

    print(f"   ✅ SID finali dopo filtri (notte + durata): {len(sid_list)}")

    return sid_list

def mostra_tabella_sid(sid_list, data_utc, flares=None):
    """Apre una finestra con la tabella dei SID rilevati."""
    import tkinter as tk
    ...
    if not sid_list:
        root = tk.Tk()
        root.title(f"Nessun SID rilevato - {data_utc.strftime('%Y-%m-%d')}")
        ...
        tk.Label(root, 
                text=f"🔍 Nessun SID rilevato per il {data_utc.strftime('%Y-%m-%d')}",
                bg='#0d0d1a', fg='#FFD700', font=('Consolas', 12, 'bold')).pack(pady=30)
        
        tk.Label(root, 
                text="Premi H per addestrare l'AI su più giorni",
                bg='#0d0d1a', fg='#888888', font=('Consolas', 10)).pack()
        
        tk.Button(root, text="Chiudi", command=root.destroy,
                 bg='#00FF99', fg='black', font=('Consolas', 10, 'bold'),
                 relief='flat', padx=25, pady=6).pack(pady=25)
        
        root.mainloop()
        return

    # --- CORRELAZIONE CON I FLARE NOAA ---
    sid_con_flare = []
    sid_senza_flare = []
    ...
    if flares:
        for sid in sid_list:
            picco = sid['picco']
            flare_associato = None
            for f in flares:
                if f['picco'].tzinfo is None:
                    f['picco'] = f['picco'].replace(tzinfo=timezone.utc)
                diff = abs((picco - f['picco']).total_seconds())
                if diff < 900:  # 15 minuti
                    flare_associato = f['classe']
                    break
            if flare_associato:
                sid['flare'] = flare_associato
                sid_con_flare.append(sid)
            else:
                sid['flare'] = '—'
                sid_senza_flare.append(sid)
    else:
        for sid in sid_list:
            if 'flare' not in sid or not sid['flare']:
                sid['flare'] = '—'
        sid_con_flare = sid_list

    # --- MOSTRA TABELLA ---
    root = tk.Tk()
    root.title(f"SID rilevati - {data_utc.strftime('%Y-%m-%d')}")
    root.attributes('-topmost', True)
    root.resizable(True, True)
    root.geometry("1050x500")  # <-- allargata per la nuova colonna
    ...
    # ===== COLONNE AGGIORNATE =====
    columns = ('Picco UTC', 'Ampiezza (dB)', 'Durata (s)', 'Area (dB·s)', 
               'Probabilità AI', 'Flare NOAA', 'Inizio UTC', 'Fine UTC')
    ...
    col_widths = {
        ...
        'Durata (s)': 90, 
        'Area (dB·s)': 110,
        'Probabilità AI': 120,  # <-- nuova colonna
        'Flare NOAA': 110, 
        'Inizio UTC': 110, 
        'Fine UTC': 110
    }
    for col in columns:
        ...
        tree.column(col, width=col_widths.get(col, 110), anchor='center')

    tree.tag_configure('con_flare', background='#c8e6c9', foreground='#1a3a1a')
    tree.tag_configure('senza_flare', background='#f5e6d3', foreground='#4a2a1a')

    for sid in sid_con_flare:
        # Estrai probabilità (se presente)
        prob = sid.get('probabilita', None)
        prob_str = f"{prob:.3f}" if prob is not None else '—'
        
        tree.insert('', 'end', values=(
            sid['picco'].strftime('%H:%M:%S'),
            f"{sid['ampiezza']:.3f}",
            f"{sid['durata']:.0f}",
            f"{sid['area']:.2f}",
            prob_str,  # <-- probabilità
            sid['flare'],
            sid['inizio'].strftime('%H:%M:%S'),
            sid['fine'].strftime('%H:%M:%S')
        ), tags=('con_flare',))

    for sid in sid_senza_flare:
        prob = sid.get('probabilita', None)
        prob_str = f"{prob:.3f}" if prob is not None else '—'
    ...
    info_label = tk.Label(root, 
                          text=f"Totale SID: {len(sid_list)}  |  Associati a flare: {len(sid_con_flare)}  |  Non associati: {len(sid_senza_flare)}",
                          font=('Consolas', 11, 'bold'), 
                          bg='#f5f5f5', 
                          fg='#1a3a1a')
    info_label.pack(pady=(6, 4))

    btn_close = tk.Button(root, text="Chiudi", command=root.destroy,
                          bg='#4CAF50', fg='white', 
                          font=('Consolas', 10, 'bold'),
...
def mostra_flare_addestrati(cartella_risposte=CARTELLA_RISPOSTE):
    """Mostra una tabella con tutti i flare usati per l'addestramento."""
    import tkinter as tk
    from tkinter import ttk
    import glob
    import json
    ...
    print(f"📂 Ricerca risposte in: {cartella_risposte}")
    
    # Cerca i file nella cartella delle risposte
    pattern = os.path.join(cartella_risposte, "risposte_flare_*.json")
    files = glob.glob(pattern)
    files.sort()
    
    print(f"   Trovati {len(files)} file di risposte")
    for f in files:
        print(f"      - {os.path.basename(f)}")
    
    if not files:
        # Controlla se esiste il modello (per messaggio informativo)
        modello_path = os.path.join(cartella_risposte, "modello_sid.pkl")
        if os.path.exists(modello_path):
            msg = "Il modello AI esiste ma i file delle risposte non sono stati trovati.\n\n"
            msg += "Potresti aver addestrato con una versione precedente che non salvava le risposte.\n"
            msg += "Premi H per rifare l'addestramento su un giorno con flare."
        else:
            msg = "Nessun file di addestramento trovato.\n\nPremi H per addestrare l'AI."
        
        root = tk.Tk()
        root.title("Nessun addestramento")
        root.attributes('-topmost', True)
        root.geometry("500x220")
        root.configure(bg='#0d0d1a')
        tk.Label(root, text=msg, bg='#0d0d1a', fg='#FFD700',
                font=('Consolas', 10), justify='left').pack(pady=20, padx=20)
        tk.Button(root, text="Chiudi", command=root.destroy,
                 bg='#00FF99', fg='black', font=('Consolas', 10, 'bold'),
    ...
    # === RACCOGLI FLARE CON DEDUPLICA ===
    flare_set = set()        # per evitare duplicati
    flare_list = []
    ...
    for file_path in files:
        try:
            ...
            data_str = os.path.basename(file_path).replace("risposte_flare_", "").replace(".json", "")
            
            for r in dati.get('risposte', []):
                picco_iso = r.get('picco')
                classe = r.get('classe', '?')
                etichetta = r.get('etichetta', 0)
                
                if not picco_iso:
                    continue
                
                try:
                    if isinstance(picco_iso, str):
                        picco_iso = picco_iso.replace('Z', '+00:00')
                        picco_dt = datetime.fromisoformat(picco_iso)
                    else:
                        picco_dt = picco_iso
                except Exception:
                    continue
                
                etichetta_str = 'SID' if etichetta == 1 else 'NON SID'
                
                # Crea una chiave unica (data, ora, classe, etichetta)
                key = (data_str, picco_dt.strftime('%H:%M:%S'), classe, etichetta_str)
                if key not in flare_set:
                    flare_set.add(key)
                    flare_list.append({
                        'data': data_str,
                        'ora': picco_dt.strftime('%H:%M:%S'),
                        'classe': classe,
                        'etichetta': etichetta_str
                    })
        except Exception as e:
    ...
    if not flare_list:
        root = tk.Tk()
        root.title("Nessun flare")
        root.attributes('-topmost', True)
        root.geometry("400x150")
        root.configure(bg='#0d0d1a')
        tk.Label(root, text="Nessun flare valido trovato nei file di risposte.",
                 bg='#0d0d1a', fg='white', font=('Consolas', 12)).pack(pady=40)
        tk.Button(root, text="Chiudi", command=root.destroy,
                 bg='#00FF99', fg='black', font=('Consolas', 10),
    ...
    # --- CREA TABELLA ---
    root = tk.Tk()
    root.title(f"Flare addestrati ({len(flare_list)})")
    root.attributes('-topmost', True)
    ...
    columns = ('Data', 'Ora', 'Classe', 'Etichetta')
    tree = ttk.Treeview(frame, columns=columns, show='headings',
    ...
    tree.tag_configure('SID', background='#c8e6c9', foreground='#1a3a1a')
    tree.tag_configure('NON SID', background='#f5d0c9', foreground='#4a1a1a')
    ...
    # --- COPIA RIGA SELEZIONATA (Ctrl+C) ---
    def copy_selected(event=None):
        selected = tree.selection()
        if not selected:
            return
        # Prende la prima riga selezionata
        item = selected[0]
        values = tree.item(item, 'values')
        # Costruisce stringa con tab come separatore (per incollare in Excel)
        copy_str = "\t".join(values)
        root.clipboard_clear()
        root.clipboard_append(copy_str)
        print("📋 Copiato: " + copy_str)
    
    # Bind Ctrl+C
    root.bind('<Control-c>', copy_selected)
    root.bind('<Control-C>', copy_selected)  # per sicurezza
    
    # --- Riepilogo in basso ---
    n_sid = sum(1 for f in flare_list if f['etichetta'] == 'SID')
    n_no = sum(1 for f in flare_list if f['etichetta'] == 'NON SID')
    
    info_frame = tk.Frame(root, bg='#0d0d1a')
    info_frame.pack(fill='x', pady=5)
    
    info = tk.Label(info_frame, 
                    text=f"Totale: {len(flare_list)}  |  SID veri: {n_sid}  |  Falsi: {n_no}",
                    bg='#0d0d1a', fg='#00FF99', font=('Consolas', 11, 'bold'))
    info.pack(side='left', padx=10)
    
    # Suggerimento per copia
    hint = tk.Label(info_frame, text="(Ctrl+C per copiare riga selezionata)",
                    bg='#0d0d1a', fg='#888888', font=('Consolas', 9))
    hint.pack(side='right', padx=10)
    
    btn = tk.Button(root, text="Chiudi", command=root.destroy,
                    bg='#00FF99', fg='black', font=('Consolas', 10, 'bold'),
...
def calcola_alba_tramonto(data_utc):
    """Calcola alba e tramonto per il punto medio del percorso VLF (Niscemi-Torino).
    Questo è il punto in cui il segnale rimbalza sulla ionosfera, quindi è qui che
    bisogna controllare se c'è il sole o meno per poter rilevare i SID."""
    osservatore           = ephem.Observer()
    # ATTENZIONE: Ho cambiato le coordinate da Torino (45.07, 7.68) 
    # al punto medio esatto del percorso radio (41.1, 11.1)
    osservatore.lat       = '41.1'
    osservatore.lon       = '11.1'
    osservatore.elevation = 100
    osservatore.date      = data_utc.strftime('%Y/%m/%d 00:00:00')
...
def scarica_dati_geomagnetici(data_utc):
    """
    Scarica e analizza dati geomagnetici per la data specificata.
    """
    cache_path = os.path.join(CARTELLA_GEOMAG, f"geomag_{data_utc.strftime('%Y-%m-%d')}.json")
    
    # Verifica se la cache esiste e quanto è vecchia
    if os.path.exists(cache_path):
        try:
            mtime = datetime.fromtimestamp(os.path.getmtime(cache_path))
            eta = (datetime.now() - mtime).total_seconds() / 3600
            if eta < 2:
                with open(cache_path, 'r') as f:
                    return json.load(f)
            else:
                print(f"   ⏰ Cache più vecchia di {eta:.1f} ore, ricarico da DONKI...")
    ...
    # === 1. CONTROLLA LA CACHE ANNUALE ===
    anno = data_utc.year
    cache_annuale = os.path.join(CARTELLA_GEOMAG, f"geomag_year_{anno}.json")
    
    if os.path.exists(cache_annuale):
        try:
            with open(cache_annuale, 'r') as f:
                events_raw = json.load(f)
            
            events = _deserializza_eventi_geomag(events_raw)
            
            # === COSTRUISCI UN DIZIONARIO VELOCITÀ CME ===
            # Chiave: ID del CME, Valore: velocità
            cme_speeds = {}
            for ev in events:
                if ev.get('type') == 'CME':
                    cme_id = ev.get('id', '')
                    velocita = ev.get('velocita_km_s', 0)
                    if cme_id and velocita > 0:
                        cme_speeds[cme_id] = velocita
                        print(f"      📊 CME {cme_id}: {velocita} km/s")
            ...
            for ev in events:
                ...
                if not isinstance(ev_date, datetime):
                    continue
                if ev_date.date() == data_utc.date():
                    print(f"   📋 Tempesta trovata nella cache annuale: Kp={ev.get('kp_max')}")
                    
                    info['kp_max'] = ev.get('kp_max')
                    info['tempesta'] = ev.get('tempesta')
                    info['gst_events'].append({
                        'startTime': ev_date,
                        'kp_max': ev.get('kp_max'),
                        'kp_values': [],
                        'linked_cmes': ev.get('linked_cmes', [])
                    })
                    
                    # === USA IL DIZIONARIO PER LE VELOCITÀ ===
                    cme_ids = ev.get('linked_cmes', [])
                    for cme_id in cme_ids:
                        velocita_cme = cme_speeds.get(cme_id, 0)
                        if velocita_cme == 0:
                            # Se non trovata, usa un valore tipico per una tempesta G3
                            if info['kp_max'] is not None and info['kp_max'] >= 7:
                                velocita_cme = 800  # Tipico per G3
                                print(f"      ⚠️ Velocità CME {cme_id} non trovata, uso default 800 km/s")
                        
                        info['cme_analyses'].append({
                            'time': ev_date,
                            'speed_km_s': velocita_cme,
                            'type': '',
                            'id': cme_id
                        })
                    break
            ...
            print(f"   ⚠️ Errore lettura cache annuale: {e}")

    # === 2. PER OGGI: SCARICA DA DONKI SE NECESSARIO ===
    oggi = datetime.now(timezone.utc).date()
    if data_utc.date() == oggi or info['kp_max'] is None:
        print(f"   📡 Scarico dati in tempo reale da DONKI...")
        try:
                ...
                for gst in gst_list:
                    try:
                        ...
                        if kp_max is not None:
                            ...
                            # Prova a ottenere le velocità dei CME
                            for cme_id in linked_cmes:
                                velocita_cme = 0
                                try:
                                    # Prova a cercare il CME specifico
                                    url_cme = f"https://kauai.ccmc.gsfc.nasa.gov/DONKI/WS/get/CMEAnalysis?activityId={cme_id}"
                                    resp_cme = requests.get(url_cme, timeout=10)
                                    if resp_cme.status_code == 200:
                                        cme_data = resp_cme.json()
                                        if cme_data and len(cme_data) > 0:
                                            velocita_cme = cme_data[0].get('speed', 0)
                                except:
                                    pass
                                
                                if velocita_cme == 0 and kp_max >= 7:
                                    velocita_cme = 800  # Valore tipico per G3
                            ...
                            print(f"   ✅ Tempesta rilevata: Kp={kp_max}, {info['tempesta']}")
                        else:
                            print(f"   ℹ️ Nessuna tempesta rilevata in tempo reale")
                        
                    except Exception as ex:
                        print(f"  Errore parsing GST: {ex}")
                        continue
        except Exception as e:
            print(f"   ⚠️ Errore download DONKI: {e}")

    # === 3. Classificazione impatto ===
    impatto, livello, note = _classifica_impatto_combinato(info)
    info['impatto'] = impatto
    info['livello'] = livello
    info['note'] = note

    # Salva in cache giornaliera
    try:
        with open(cache_path, 'w') as f:
            json.dump(info, f, indent=2, default=str)
        print(f"   💾 Cache aggiornata: {os.path.basename(cache_path)}")
    except Exception as e:
...
def _aggiorna_status_pannello(fig, grb_ril, info_geomag=None):
    ...
    # Parte geomagnetica
    if info_geomag:
        kp_max = info_geomag.get('kp_max', None)
        tempesta = info_geomag.get('tempesta', None)
        cme_analyses = info_geomag.get('cme_analyses', [])
        note = info_geomag.get('note', [])
        
        # SE C'È UNA TEMPESTA (kp >= 5), MOSTRALA SEMPRE
        if kp_max is not None and kp_max >= 5:
            righe.append(f"⚠ TEMPESTA {tempesta.upper() if tempesta else 'GEOMAGNETICA'} (Kp={kp_max:.1f})")
            colore = '#FF0000'
            
            # === CORREZIONE 1: Mostra SOLO i CME con velocità > 0 (o al massimo 1) ===
            cme_mostrati = 0
            for cme in cme_analyses:
                vel = cme.get('speed_km_s', 0)
                if vel > 0 and cme_mostrati == 0:  # Mostra solo il primo CME con velocità
                    righe.append(f"  · CME in arrivo ({vel:.0f} km/s)")
                    cme_mostrati += 1
                elif cme_mostrati == 0:  # Se nessun CME ha velocità, mostrane uno generico
                    righe.append("  · CME in arrivo")
                    cme_mostrati += 1
                # Ignora gli altri CME per evitare duplicati
            
            # === CORREZIONE 2: Mostra le note, ma EVITA duplicati con i CME ===
            for n in note[:2]:
                # Salta le note che parlano di CME o tempesta (già mostrate)
                if "CME" in n or "Tempesta" in n or "Kp" in n:
                    continue
                righe.append(f"  · {n}")
                    
        elif info_geomag.get('impatto', False):
            # Altrimenti, usa la valutazione combinata
            ...
            colore = '#FF0000'
            for n in note[:2]:
                if "CME" not in n and "Tempesta" not in n:
                    righe.append(f"  · {n}")
        else:
            if not grb_ril:
                colore = '#006600'
...
def disegna_grafico():
    ...
    timestamps, valori = leggi_csv_sid(csv_path)
        # --- DEBUG: stampa i primi timestamp per verificare il fuso orario ---
    if timestamps:
        print(f"\n🔍 VERIFICA TIMESTAMP (primi 5 campioni):")
        for i in range(min(5, len(timestamps))):
            print(f"   {i+1}. {timestamps[i].strftime('%Y-%m-%d %H:%M:%S')} UTC")
        
        # Stampa anche alba/tramonto per confronto
        alba, tramonto = calcola_alba_tramonto(data_utc)
        if alba and tramonto:
            print(f"   Alba: {alba.strftime('%H:%M')} UTC")
            print(f"   Tramonto: {tramonto.strftime('%H:%M')} UTC")
    if not timestamps:
        print("Nessun dato nel file CSV VLF.")
        return
    
    # Salva i dati grezzi nello stato per la rilevazione SID (tasto T)
    stato['timestamps'] = timestamps
    stato['valori_grezzi'] = valori
    ...
    # --- CALCOLO RUMORE DI FONDO (verifica SNR) - IN dB ---
    # --- METRICA PER IL FILTRO HAMPEL (1 minuto, notte) ---
    if len(timestamps) > 0 and finestra_smooth <= 15:
        timestamps_arr = np.array(timestamps)
        # Scegli una finestra notturna di 1 minuto (01:00-01:01 UTC)
        inizio = data_utc.replace(hour=1, minute=0, second=0)
        fine = data_utc.replace(hour=1, minute=1, second=0)
        mask_corto = (timestamps_arr >= inizio) & (timestamps_arr <= fine)
        
        if np.sum(mask_corto) > 30:
            # Rumore sul segnale smoothing (dopo il filtro)
            valori_arr = np.array(valori_smooth)
            rumore_dopo = np.std(valori_arr[mask_corto])
            
            # Rumore sul segnale grezzo (prima del filtro)
            valori_grezzi = np.array(valori)
            rumore_prima = np.std(valori_grezzi[mask_corto])
            
            print(f"\n📊 EFFICACIA FILTRO HAMPEL (1:00-1:01 UTC, finestra={finestra_smooth}s):")
            print(f"   Rumore prima del filtro (solo dati grezzi): {rumore_prima:.4f} dB")
            print(f"   Rumore dopo il filtro (SG + Hampel):       {rumore_dopo:.4f} dB")
            print(f"   Riduzione del rumore impulsivo:            {(1 - rumore_dopo/rumore_prima)*100:.1f}%")
            print(f"   Numero campioni:                           {np.sum(mask_corto)}")
    ...
    ax.set_title(
        f"{data_utc.strftime('%Y-%m-%d UTC')}\n" 
        f"Segnale VLF NSY - Flusso X-ray GOES - Flare Solari - GRB{smooth_title} - New Filter\n"
        f"A = prec.  |  D = succ.  |  I = data  |  M = flare Man  |  F = flare ON  |  G = GRB ON |  Y = Eventi Geomagnetici |\n"
        f"B = GRB  |  Shift+B = tutti GRB  | H = AI addestr  |  "
        f"S / V = smooth lista / libero  |  U = scala [{scala_stato}]  |  L = SID usati da AI  |  "
        f"X = X-ray [{xray_stato}]  |  T = Ricerca SID con AI  |  R = Refresh  |  P = Soglia probab  |  Q = esci",
        color='white', fontsize=11, fontweight='bold', pad=18, y=0.99
    )
...
def on_key(event):
    ...
    if key_norm == 'r':
        ...
        parent = getattr(event.canvas, 'window', None)  # finestra principale Tk
        data_str = stato['data'].strftime("%Y-%m-%d")
        try:
            classe = tksd.askstring("Flare manuale", f"Classe flare (es. C5.3, M1.2, X1.0)\nData: {data_str}", parent=parent)
            if not classe:
                return
            classe = classe.strip().upper()
            ora_picco = tksd.askstring("Flare manuale", "Ora picco UTC (HH:MM):", parent=parent)
            if not ora_picco:
                return
            ora_picco = ora_picco.strip()
            ora_inizio = tksd.askstring("Flare manuale", f"Ora inizio UTC (HH:MM)\n[lascia vuoto per usare {ora_picco}]:", parent=parent)
            ora_inizio = ora_inizio.strip() if ora_inizio and ora_inizio.strip() else ora_picco
            ora_fine = tksd.askstring("Flare manuale", f"Ora fine UTC (HH:MM)\n[lascia vuoto per usare {ora_picco}]:", parent=parent)
            ora_fine = ora_fine.strip() if ora_fine and ora_fine.strip() else ora_picco
            nota = tksd.askstring("Flare manuale", "Nota opzionale:", parent=parent)
    ...
    #   salva_png()
        ...
        print("\n🔍 Rilevazione SID in corso (modello AI su flare)...")
        
        if 'timestamps' not in stato or 'valori_grezzi' not in stato:
            print("⚠️ Nessun dato disponibile.")
            return
        
        modello_path = os.path.join(CARTELLA_RISPOSTE, "modello_sid.pkl")
        modello = sid_ai.carica_modello(path=modello_path)

        if modello is None:
            print("⚠️ Modello AI non trovato.")
            risp = input("   Avviare l'addestramento? [S/N]: ").strip().lower()
            if risp == 's':
                # Simula la pressione del tasto H
                class FakeEvent:
                    key = 'h'
                on_key(FakeEvent())
            return
            
        timestamps = stato['timestamps']
        valori = stato['valori_grezzi']
        data_utc = stato['data']
        
        # Carica flare NOAA
        flares_noaa = sid_ai.leggi_flare_cache(data_utc)
        
        # Carica flare manuali (dal tuo main)
        flares_manuali = carica_flare_manuali()
        flares_manuali = [f for f in flares_manuali if f['picco'].date() == data_utc.date()]
        
        # Rileva SID
        soglia_corrente = sid_ai.SOGLIA_PROBABILITA_DEFAULT
        print(f"   📊 Soglia probabilità: {soglia_corrente}")

        sid_detected = sid_ai.rileva_sid_con_modello(
            timestamps, valori, data_utc, modello,
            soglia_prob=soglia_corrente,   # <--- usa la variabile aggiornata
            flares_noaa=flares_noaa,
            flares_manuali=flares_manuali
        )
        
        # Prepara i flare per la tabella (anche se vuota)
        for s in sid_detected:
            if 'flare' not in s or not s['flare']:
                s['flare'] = '—'
        
        flares_totali = list(flares_noaa) + flares_manuali
        
        # Mostra SEMPRE la tabella (anche se vuota)
        mostra_tabella_sid(sid_detected, stato['data'], flares_totali)
        
        if sid_detected:
            print(f"   ✅ Trovati {len(sid_detected)} SID.")
        else:
            print("   ❌ Nessun SID rilevato.")


    elif key_norm == 'h':
        # --- ADDESTRAMENTO INTERATTIVO ---
        print("\n🧠 Addestramento AI interattivo...")
        
        import tkinter as tk
        from tkinter import simpledialog

        if 'timestamps' not in stato or 'valori_grezzi' not in stato:
            print("⚠️ Nessun dato disponibile. Carica prima un giorno.")
            return
        ...
        # Chiedi classe soglia
        root = tk.Tk()
        root.withdraw()
        root.attributes('-topmost', True)
        classe_soglia = simpledialog.askstring(
            "Soglia flare",
            "Inserisci la classe minima dei flare da considerare\n"
            "Esempi: C1, C2, M1, M5, X1\n"
            "(I flare inferiori a questa soglia verranno ignorati per il training)",
            parent=root
        )
        root.destroy()
        
        if not classe_soglia:
            print("❌ Addestramento annullato.")
            return
        
        classe_soglia = classe_soglia.strip().upper()
        print(f"📌 Classe soglia: {classe_soglia}")
        
        # Carica i flare NOAA (dal cache)
        flares = sid_ai.leggi_flare_cache(data_utc)
        if not flares:
            print("⚠️ Nessun flare NOAA trovato per questo giorno.")
            # Chiedi se continuare lo stesso con Tkinter
            root2 = tk.Tk()
            root2.withdraw()
            root2.attributes('-topmost', True)
            risp = simpledialog.askstring(
                "Nessun flare",
                "Nessun flare NOAA trovato per questo giorno.\nContinuare lo stesso? [S/N]",
                parent=root2
            )
            root2.destroy()
            if not risp or risp.lower() != 's':
                print("❌ Addestramento annullato.")
                return
            # Se l'utente dice Sì, passa flares=None (o [] ) alla funzione
            flares_noaa = None
        else:
            flares_noaa = flares
        
        # Lancia l'addestramento (NON passare ax e fig)
        global modello_ai
        modello, risposte = sid_ai.addestra_interattivo(
            timestamps, valori, data_utc,
            classe_soglia,
            ax=None,
            fig=None,
            finestra_smooth=stato['smooth'],
            cartella_risposte=CARTELLA_RISPOSTE  # <-- PASSA LA CARTELLA
        )
        ...
        if modello is not None:
            ...
            print("❌ Addestramento fallito o annullato.")

    elif key_norm == 'p':
        # Modifica soglia probabilità (tasto P)
        nuova_soglia = sid_ai.modifica_soglia_probabilita()
        print(f"📊 Soglia probabilità aggiornata a {nuova_soglia}")

    elif key_norm == 'l':
        # --- MOSTRA TABELLA FLARE ADDESTRATI ---
        print("\n📋 Caricamento flare di addestramento...")
        mostra_flare_addestrati(CARTELLA_RISPOSTE)
...
print("  P  -> Modifica Soglia Probabiltà AI")
...
print("  H  -> Addestramento AI interattivo (sul giorno corrente)")
print("  T  -> Rileva SID con modello AI (premi H prima per addestrare)")
print("  P  -> Modifica soglia probabilità (default 0.5)")
print("  Shift+B -> Anno GRB (tutti i GRB)")
print("  L  -> Mostra tabella flare addestrati (con etichette)")
print()
...
# Sposta la toolbar in basso e abilita la barra di stato
try:
    if hasattr(fig.canvas.manager, 'toolbar'):
        # Per backend TkAgg
        fig.canvas.manager.toolbar.pack(side='bottom')
        # Abilita la barra di stato (se non lo è già)
        fig.canvas.manager.toolbar.config(relief='sunken')
except Exception:
    pass
...
# Carica modello AI (se esiste)
modello_ai = sid_ai.carica_modello()
if modello_ai is not None:
    print("✅ Modello AI caricato.")
else:
    print("ℹ️ Nessun modello AI trovato. Premi H per addestrare.")
...
# --- Posizionamento finestra (versione robusta) ---
_manager = plt.get_current_fig_manager()
if hasattr(_manager, 'window'):
    _win = _manager.window
    # Imposta geometria iniziale
    try:
        _win.geometry(f"{_SW}x{_WH}+0+{_SH - _WH}")
    except:
        pass
    # Prova a collegare gli eventi solo se il metodo bind esiste
    if hasattr(_win, 'bind'):
        def _salva_geometria(event=None):
            try:
                geo = _win.geometry()
                # memorizza in una variabile globale o lista
                if not hasattr(_salva_geometria, 'geo'):
                    _salva_geometria.geo = geo
                else:
                    _salva_geometria.geo = geo
            except:
                pass
        def _applica_geometria(event=None):
            try:
                if hasattr(_salva_geometria, 'geo'):
                    _win.geometry(_salva_geometria.geo)
            except:
                pass
        ...
        print("⚠️ Impossibile gestire il ridimensionamento automatico della finestra.")
