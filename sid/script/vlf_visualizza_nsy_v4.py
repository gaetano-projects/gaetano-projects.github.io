...
stato = {
    ...
    'scala_lineare': False,   # False = dB (default), True = lineare (tasto U)
}
...
def disegna_grafico():
    ...
    # --- Conversione scala lineare (tasto U) ---
    # I valori raw sono in dBm (potenza): conversione con /10 → mW normalizzati
    # Riferimento arbitrario: la curva è in unità proporzionali alla tensione
    # usando /20, coerente con la convenzione di Loudet (V) per confronto visivo.
    if stato['scala_lineare']:
        valori_plot = [10 ** (v / 20.0) for v in valori_smooth]
    else:
    ...
    if stato['scala_lineare']:
        ax.set_ylabel('Segnale VLF (lineare, u.a.)', color='#00FF99', fontsize=11)
    else:
...
def salva_png():
    ...
    output_png = os.path.join(
        CARTELLA_STORICO,
        f"VLF_flare_{data_utc.strftime('%Y-%m-%d')}{smooth_suffix}{xray_suffix}{scala_suffix}.png"
    )
...
# =============================================================
# DIALOGO SMOOTHING (riutilizzabile da tastiera durante visualizzazione)
# =============================================================

def apri_dialogo_smooth_lista():
    """
    Finestra con bottoni per i valori standard di smoothing (tasto S).
    Restituisce il valore scelto oppure None se annullato.
    """
    import tkinter as tk
    ...
    tk.Label(root, text="Scegli la finestra di smoothing",
             bg='#0d0d1a', fg='#00FF99',
             font=('Consolas', 11, 'bold')).pack(pady=(14, 4), padx=20)

    corrente = stato['smooth']
    tk.Label(root, text=f"Valore attuale: {corrente} s",
             bg='#0d0d1a', fg='#aaaacc',
    ...
    frame_std.pack(pady=(0, 14), padx=20)
    ...
    for i, val in enumerate(VALORI_SMOOTH_STANDARD):
        bg_i = '#00FF99' if val == corrente else '#1e1e3a'
        fg_i = 'black'   if val == corrente else 'white'
        b = tk.Button(frame_std,
                      text=etichette.get(val, str(val)),
                      width=8, height=2,
                      bg=bg_i, fg=fg_i,
                      font=('Consolas', 10),
                      relief='flat', bd=1)
        b.grid(row=0, column=i, padx=5, pady=4)
        b.config(command=lambda v=val, btn=b: seleziona(v, btn))

    tk.Button(root, text="Annulla", command=root.destroy,
              bg='#333355', fg='white', font=('Consolas', 9),
              relief='flat', padx=10, pady=3).pack(pady=(0, 12))
...
def apri_dialogo_smooth_manuale():
    """
    Finestra con campo testo per valore libero di smoothing (tasto V).
    Restituisce il valore scelto oppure None se annullato.
    """
    import tkinter as tk
...
def on_key(event):
    ...
    if tasto == 'r':
        ...
        modo = "LINEARE (u.a.)" if stato['scala_lineare'] else "LOGARITMICA (dB)"
        print(f"Scala asse Y: {modo}")
        disegna_grafico()
...
for tasto in ['p','q','s','v','c','a','d','g','h','r','k','l','o','f','x','u']:
